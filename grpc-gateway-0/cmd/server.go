...
import (
	...
	"github.com/lovemew67/project-misc/grpc-gateway-0/controllerv1"
...
func NewAPIServerCmd() *cobra.Command {
	...
	var apiServerCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			defer func() {
				cornerstone.Infof(systemCtx, "[%s] shuting down grpc gateway", cornerstone.GetAppName())
				// grpcGateway.Close()
			}()
			go func() {
				cornerstone.Infof(systemCtx, "[%s] grpc gateway is running and listening port: %s", cornerstone.GetAppName(), grpcGatewayPort)
				if errServe := grpcGateway.ListenAndServe(); errServe != nil {
					cornerstone.Panicf(systemCtx, "[%s] grpc gateway error: %s", cornerstone.GetAppName(), errServe)
				}
			}()
